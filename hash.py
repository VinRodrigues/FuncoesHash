...
for mensagem, sha256_hash, md5_hash in mensagens:
    sha256_generated = hashlib.sha256(mensagem.encode('utf-8')).hexdigest()
    md5_generated = hashlib.md5(mensagem.encode('utf-8')).hexdigest()
    
    print("Mensagem:", mensagem)
    
    if sha256_generated != sha256_hash:
        print(Fore.RED + "Diferença no SHA256 encontrado!")
        print("Hash esperado:", sha256_hash)
        print("Hash encontrado:", sha256_generated)
    else:
        print(Fore.GREEN + "SHA256: Correto")
        
    if md5_generated != md5_hash:
        print(Fore.RED + "Diferença no MD5 encontrado!")
        print("Hash esperado:", md5_hash)
        print("Hash encontrado:", md5_generated)
    else:
        print(Fore.GREEN + "MD5: Correto")
